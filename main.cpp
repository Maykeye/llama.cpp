...
std::string run_prompt(gpt_params &params, llama_model &model, gpt_vocab &vocab, std::mt19937 &rng, const std::string &prompt)
{
    std::string result;
    int n_past = 0;
...
bool check_op(const std::string &line, const std::string &expected_prefix, std::string &operand)
{
    std::string real_prefix = line.substr(0, expected_prefix.length());
    if (expected_prefix == real_prefix)
    {
        operand = line.substr(expected_prefix.length());
        return true;
    }
    return false;
}
...
int main(int argc, char **argv)
{
    ...
    {
        if (check_op(next_line, ".quit", operand) || check_op(next_line, ".quit$", operand))
        {
            break;
        }
        std::cerr << next_line << "\n";

        if (check_op(next_line, ".echo$", operand))
        {
            std::cerr << operand << "\n";
        }
        else if (check_op(next_line, ".app$", operand))
        {
            next_prompt += operand;
        }
        else if (check_op(next_line, ".appn$", operand))
        {
            next_prompt += operand;
            next_prompt += "\n";
        }
        else if (check_op(next_line, ".gen$", operand))
        {
            auto response = run_prompt(params, model, vocab, rng, next_prompt);
            size_t pos = 0;
            while ((pos = response.find('\n')) != std::string::npos)
            {
                auto line = response.substr(0, pos);
                std::cout << ".appn$" << line << "\n";
                response.erase(0, pos + 1);
            }
            if (response.length() > 0)
            {
              std::cout << ".app$" << response << "\n";
            }
            std::cout << ".done$\n";
            std::flush(std::cout);
            next_prompt = "";
